...
site = ['alt','asc','ask','bhd','bmw','brw','cgo',
        'crz','ice','izo','key','kum','mhd','mid','mlo',
        'nwr','psa','rpb','sey','shm','smo','spo','sum','syo',
        'ush','uum','wis','wlg','zep','uta']


lati=[43,21,28,12,31,40,12,11,38,30,29,27,36,30,27,33, 6,26,21,36,19, 0,41, 5, 9,34,30,32,42,32]
loni=[23,33,37,71,23, 5,65,46,32,33,20, 5,34, 0, 5,15,23,24,47,63, 2,48,28,44,22,58,43,56,38,13]
...
Se = np.zeros([len(yy),len(yy)])
for ii in np.arange(len(yy)): Se[ii,ii] = se_err
...
xx = xa + multi_dot([inv(multi_dot([tranjac,invse,jacobian])+invsa),tranjac,invse,(yy-mix)])
